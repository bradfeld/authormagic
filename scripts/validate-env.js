...
const REQUIRED_VARS = {
  ...
  CLERK_SECRET_KEY: {
    required: true,
    pattern: /^sk_(test|live)_[A-Za-z0-9_-]+$/,
...
function validateEnvironment(envFile = '.env.local') {
  ...
  if (
    ...
    console.log(
      '📦 Running in deployment environment - using injected environment variables',
    );
    envVars = process.env;
  } else {
  ...
  for (const [varName, config] of Object.entries(REQUIRED_VARS)) {
    if (!envVars[varName]) {
      errors.push(
        `❌ Missing required variable: ${varName} (${config.description})`,
      );
      isValid = false;
  ...
  for (const [wrong, correct] of Object.entries(COMMON_MISTAKES)) {
    if (envVars[wrong] && !envVars[correct]) {
      warnings.push(
        `⚠️  Found '${wrong}' but expected '${correct}' - this may cause runtime errors`,
      );
    }
  }
  ...
  for (const varName of Object.keys(envVars)) {
    if (COMMON_MISTAKES[varName]) {
      warnings.push(
        `⚠️  Variable '${varName}' should be renamed to '${COMMON_MISTAKES[varName]}'`,
      );
    }
  }
